...
- **Read the Java reference** — for each task involving protocol or game logic, read the equivalent vanilla Java class in `mc-server-ref/decompiled/net/minecraft/`. Focus on client-side behavior. Understand the algorithm, then rewrite idiomatically in Rust — never transliterate.

### 2. Planning

- Create SQL todos for each task in the phase document, with dependencies.
- Order tasks by dependency chain — implement foundations first (types, codecs, structs), then systems, then wiring, then tests.
...
- Connect packet handlers, event dispatchers, or client lifecycle hooks as specified.
...
- Errors: `thiserror` in libraries, `anyhow` only in examples/tests. Use `?` + `.context()`.
